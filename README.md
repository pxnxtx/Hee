...
_G["Clone 1"] = true      -- เปิด Clone 1
_G["Clone 2"] = true      -- เปิด Clone 2  
_G["Clone 3"] = true      -- เปิด Clone 3
_G["Clone 4"] = true      -- เปิด Clone 4
...
function AttackModule:AttackEnemy(enemyHRP, extra)
	if enemyHRP and enemyHRP.Parent and enemyHRP.Parent:FindFirstChildOfClass("Humanoid") then
		for i = 0,3 do
			RegisterAttack:FireServer(i)
		end
		RegisterHit:FireServer(enemyHRP, extra or {})
	end
end

function AttackModule:AttackAllNearby()
	local hitList = {}
	local nearest = nil
	local shortest = 999
	for _, enemy in ipairs(enemyFolder:GetChildren()) do
		...
		if hum and hrp and hum.Health > 0 then
			local dist = (client.Character and client:DistanceFromCharacter(hrp.Position)) or 999
			if dist < 60 then
				if dist < shortest then
					shortest = dist
					nearest = hrp
				else
					table.insert(hitList, {enemy, hrp})
				end
			end
		end
	end
	if nearest then
		self:AttackEnemy(nearest, hitList)
	end
end

function Attack()
	AttackModule:AttackAllNearby()
end

local Players, ReplicatedStorage, RunService, Workspace =
...
local LocalPlayer = Players.LocalPlayer
local Net = ReplicatedStorage:WaitForChild("Modules"):WaitForChild("Net")
local RegisterHit, RegisterAttack, ShootGunEvent =
    Net:WaitForChild("RE/RegisterHit"),
    Net:WaitForChild("RE/RegisterAttack"),
    Net:WaitForChild("RE/ShootGunEvent")
...
do
    ...
    pcall(function()
        local success, env = pcall(getsenv, LocalScript)
        if success and env then HIT_FUNCTION = env._G.SendHitsToServer end
    end)
end

-- Damage Aura Function
local function DamageAura()
    ...
    if enemy then
        ...
        local hrp = enemy:FindFirstChild("HumanoidRootPart")
        if hum and hrp and hum.Health > 0 and (hrp.Position - root.Position).Magnitude <= 65 then
            if HIT_FUNCTION then
                HIT_FUNCTION(hrp, {{enemy, hrp}, math.random()})
            else
                RegisterHit:FireServer(hrp, {{enemy, hrp}, math.random()})
            end
        end
    end
    ...
    if player and player.Name ~= LocalPlayer.Name then
        ...
        if hum and hrp and hum.Health > 0 and (hrp.Position - root.Position).Magnitude <= 65 then
            if HIT_FUNCTION then
                HIT_FUNCTION(hrp, {{player, hrp}, math.random()})
            else
                RegisterHit:FireServer(hrp, {{player, hrp}, math.random()})
            end
        end
    end
end

-- Instant Fast Attack Function
local function InstantFastAttack()
    local char = LocalPlayer.Character
    if not char then return end
...
-- Bring Fast Attack Function  
local function BringFastAttack()
    local char = LocalPlayer.Character
    if not char then return end
    local root = char:FindFirstChild("HumanoidRootPart")
    if not root then return end
    ...
    if enemy then
        ...
        if hum and hrp and hum.Health > 0 then
            ...
            hrp.CFrame = root.CFrame + root.CFrame.LookVector * 3
...
-- Lightweight Clone Attack Function
local function CloneAttack(target)
    if not target then return end
    
    if HIT_FUNCTION then
        HIT_FUNCTION(target, {{target.Parent, target}, math.random()})
    else
        RegisterHit:FireServer(target, {{target.Parent, target}, math.random()})
    end
end

-- Main Multi-Clone System (Single Loop)
local cloneCount = 0
RunService.Heartbeat:Connect(function()
    if not C()["Fast Attack"] then return end
    ...
    local hum = enemy:FindFirstChild("Humanoid")
    local hrp = enemy:FindFirstChild("HumanoidRootPart")
    if not (hum and hrp and hum.Health > 0 and (hrp.Position - root.Position).Magnitude <= 65) then return end
    
    -- Rotate between clones to reduce ping
    cloneCount = cloneCount + 1
    if cloneCount > 4 then cloneCount = 1 end
    
    pcall(function()
        if cloneCount == 1 and _G["Clone 1"] then
            RegisterAttack:FireServer(0)
            CloneAttack(hrp)
        elseif cloneCount == 2 and _G["Clone 2"] then
            RegisterAttack:FireServer(0)
            CloneAttack(hrp)
        elseif cloneCount == 3 and _G["Clone 3"] then
            RegisterAttack:FireServer(0)
            CloneAttack(hrp)
        elseif cloneCount == 4 and _G["Clone 4"] then
            RegisterAttack:FireServer(0)
            CloneAttack(hrp)
        end
    end)
